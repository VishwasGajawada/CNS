...
int binToDec(string bin) {
    int dec = 0;
    for(int i=0;i<4;i++) dec = dec*2 + (bin[i]-'0');
    return dec;
}
...
string binToHex(string bin) {
    ...
    for(int i=0;i<len;i+=4) {
        int dec = binToDec(bin.substr(i, 4));
        if(dec<10) hex+=('0'+dec);
        else hex+=('A'+dec-10);
    }
    return hex;
...
string decimaltohex(int64 deci) {
    ...
    string hexstring = binToHex(EQBIN);
    return hexstring;
}
...
string SHA512(string myString) {
    ...
    // Find the count of chunks of size 1024 each
    ...
    for (int block = 0; block < blocksnumber; ++block) { 
        // Divide the current string into 80 blocks size 16 each
        separator(Blocks[block]);
        ...
        for (int i=0; i<80; i++) {
            int64 T1 = H + Ch(E, G, H) + sigmaE(E) + Message[i] + Constants[i];
            int64 T2 = sigmaA(A) + maj(A, B, C);
            H = G;
            G = F;
            F = E;
